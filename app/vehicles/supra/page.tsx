...
import {
  ArrowLeft, Zap, Shield, Gauge, Disc, MapPin,
  ChevronRight, MessageCircle, Camera,
} from "lucide-react";
import styles from "../supreme/supreme.module.css";

const keyFeatures = [
  { icon: Zap,    label: "120 Km+ True Range", desc: "Real-world verified range"    },
  { icon: Disc,   label: "Disc / Disc Brakes", desc: "Front & rear disc braking"    },
  { icon: Gauge,  label: "Built-In Motor",     desc: "High-performance BLDC motor"  },
  { icon: Shield, label: "Anti-Theft Alarm",   desc: "Smart alarm protection"       },
  { icon: MapPin, label: "Road Service",       desc: "Pan-India service network"    },
];
...
const galleryImages = [
  { src: "/vehicles/supra/Supra-front.webp",  label: "Front View"  },
  { src: "/vehicles/supra/Supra-Rear.webp",   label: "Rear View"   },
  { src: "/vehicles/supra/Supra-side.webp",   label: "Side View"   },
  { src: "/vehicles/supra/Supra-Side.png",    label: "Side View 2" },
];
      ...
      <section ref={heroRef} className={styles.hero}>
        <motion.div style={{ opacity: heroOpacity }} className={styles.heroLeft}>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.15, ease: [0.16, 1, 0.3, 1] }} className={styles.heroHeadBlock}>
            <h1 className={styles.heroTitle}>Supra</h1>
            <p className={styles.heroTagline}>All-round performance.</p>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.35 }} className={styles.priceRow}>
              <span className={styles.priceAmount}>&#8377;70,000</span>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className={styles.heroDesc}>
            The Supra refuses to compromise. Sport-tuned suspension, 120&nbsp;km+ verified range, disc brakes and an aggressive stance — built for those who want it all.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.55 }} className={styles.heroCtas}>
            <a href="https://wa.me/919876543210?text=Hi%2C+I%27m+interested+in+the+Supra+EV" target="_blank" rel="noopener noreferrer" className={styles.ctaWhatsapp}>
        ...
        <div className={styles.heroRight}>
          <div className={styles.tiltScene} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <motion.div ref={cardRef} style={{ scale: heroImgScale, rotateX, rotateY }} className={styles.heroImgInner}>
              <Image src="/vehicles/supra/supra family.webp" alt="Supra EV" fill priority className={styles.heroImg} sizes="55vw" quality={90} />
      ...
      <section className={styles.specsSection}>
        <div className="container-xl">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.whoForCard}>
            ...
            <p className={styles.whoForText}>The go-getter who wants maximum performance and real-world range — a premium scooter without the compromise of RTO registration or fuel bills.</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.galleryHeading}>
            <div>
              ...
              <h2 className={styles.galleryTitle}>See the <span className={styles.accent}>Supra</span> up close.</h2>
            </div>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.ctaBox}>
            <div className={styles.ctaBoxLeft}>
              ...
              <h2 className={styles.ctaBoxTitle}>Book your <span className={styles.accent}>Supra</span> today.</h2>
