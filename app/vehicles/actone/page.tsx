...
import { ArrowLeft, Bluetooth, Navigation, Gauge, Shield, Bell, ChevronRight, MessageCircle, Camera } from "lucide-react";
import styles from "../supreme/supreme.module.css";

const keyFeatures = [
  { icon: Bluetooth,  label: "Bluetooth Connect", desc: "Connect your smartphone"       },
  { icon: Navigation, label: "Cruise Control",    desc: "Effortless constant speed"     },
  { icon: Gauge,      label: "Low / High Speed",  desc: "Dual speed mode selector"      },
  { icon: Shield,     label: "Anti-Theft Alarm",  desc: "Built-in alarm protection"     },
  { icon: Bell,       label: "Around Service",    desc: "Pan-India service network"     },
];

const specGroups = [
  {
    group: "Electrical", color: "#F0A500",
    items: [
  ...
  {
    group: "Motor / Chassis", color: "#F0A500",
    items: [
      { label: "Motor",            value: "BLDC High Speed Motor"              },
      { label: "Speed",            value: "Low Speed / High Speed"             },
  ...
  {
    group: "Brake / Tyre / Weight", color: "#F0A500",
    items: [
      { label: "Brake System", value: "Disc / Drum"          },
...
const galleryImages = [
  { src: "/vehicles/actone/Actone-Front-1.jpeg",      label: "Front View"  },
  { src: "/vehicles/actone/Actone-Rear.webp",         label: "Rear View"   },
  { src: "/vehicles/actone/Actone-Side-1.webp",       label: "Side View"   },
  { src: "/vehicles/actone/Actone-Side-1 (1).webp",   label: "Side View 2" },
];
...
export default function ActonePage() {
  ...
  const rawRotateX = useMotionValue(0); const rawRotateY = useMotionValue(0);
  const rawGlareX  = useMotionValue(50); const rawGlareY  = useMotionValue(50);
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    rawRotateX.set((y - 0.5) * -16); rawRotateY.set((x - 0.5) * 16);
    rawGlareX.set(x * 100); rawGlareY.set(y * 100);
  };
  const handleMouseLeave = () => { rawRotateX.set(0); rawRotateY.set(0); rawGlareX.set(50); rawGlareY.set(50); };
      ...
      <section ref={heroRef} className={styles.hero}>
        <motion.div style={{ opacity: heroOpacity }} className={styles.heroLeft}>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.15, ease: [0.16, 1, 0.3, 1] }} className={styles.heroHeadBlock}>
            <h1 className={styles.heroTitle}>Actone</h1>
            <p className={styles.heroTagline}>Long ride, easy charge.</p>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.35 }} className={styles.priceRow}>
              <span className={styles.priceAmount}>&#8377;60,000</span>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className={styles.heroDesc}>
            The smart long-ranger. A Bluetooth-connected BLDC motor with cruise control, low &amp; high speed modes — the features that make the commute smarter at a great price.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.55 }} className={styles.heroCtas}>
            <a href="https://wa.me/919876543210?text=Hi%2C+I%27m+interested+in+the+Actone+EV" target="_blank" rel="noopener noreferrer" className={styles.ctaWhatsapp}>
        ...
        <div className={styles.heroRight}>
          <div className={styles.tiltScene} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <motion.div ref={cardRef} style={{ scale: heroImgScale, rotateX, rotateY }} className={styles.heroImgInner}>
              <Image src="/vehicles/actone/actoneev.webp" alt="Actone EV" fill priority className={styles.heroImg} sizes="55vw" quality={90} />
      ...
      <section className={styles.specsSection}>
        <div className="container-xl">
          ...
          <div className={styles.featureGrid}>
            {keyFeatures.map(({ icon: Icon, label, desc }, i) => (
              <motion.div key={label} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, delay: i * 0.08 }} className={styles.featureCard}>
                <div className={styles.featureCardTop}><div className={styles.featureIcon}><Icon size={20} /></div><span className={styles.featureIncluded}>Included</span></div>
                ...
                <div className={styles.tabPanelGrid}>
                  {specGroups[activeTab].items.map((item, ii) => (
                    <div key={item.label} className={styles.tabItem}>
                      ...
                      <div className={styles.tabItemBody}><span className={styles.tabItemLabel}>{item.label}</span><span className={styles.tabItemValue}>{item.value}</span></div>
                    </div>
                  ))}
                </div>
              </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.whoForCard}>
            ...
            <p className={styles.whoForText}>The small-to-medium family commuter who rides further each day and wants longer range and easy charging at a value price.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className={styles.gallerySection}>
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.galleryHeading}>
            <div>
              ...
              <h2 className={styles.galleryTitle}>See the <span className={styles.accent}>Actone</span> up close.</h2>
            </div>
        ...
        <div className={styles.galleryGrid}>
          <div className={styles.gallerySpotlight}>
            <AnimatePresence mode="wait">
              <motion.div key={activeGallery} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.4 }} className={styles.gallerySpotlightInner}>
                ...
                <div className={styles.gallerySpotlightBottom}>
                  ...
                  <div className={styles.galleryDots}>{galleryImages.map((_, i) => (<button key={i} onClick={() => setActiveGallery(i)} className={styles.galleryDot} data-active={activeGallery === i} aria-label={`View image ${i + 1}`} />))}</div>
                </div>
              </motion.div>
            </AnimatePresence>
          </div>
          <div className={styles.galleryPanels}>
            {galleryImages.map((img, i) => (
              <motion.button key={img.src} initial={{ opacity: 0, x: 32 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }} onClick={() => setActiveGallery(i)} className={styles.galleryPanelItem} data-active={activeGallery === i} aria-label={`View ${img.label}`}>
                ...
                <div className={styles.galleryPanelOverlay} /><span className={styles.galleryPanelLabel}>{img.label}</span>
              </motion.button>
            ))}
          </div>
        </div>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.ctaBox}>
            <div className={styles.ctaBoxLeft}>
              ...
              <h2 className={styles.ctaBoxTitle}>Book your <span className={styles.accent}>Actone</span> today.</h2>
