...
import { ArrowLeft, Usb, RotateCcw, Disc, Shield, Bell, ChevronRight, MessageCircle, Camera } from "lucide-react";
import styles from "../supreme/supreme.module.css";

const keyFeatures = [
  { icon: Usb,      label: "USB Charging Port", desc: "Stay charged on the go"        },
  { icon: RotateCcw,label: "Reverse Gear",      desc: "Convenient reverse control"   },
  { icon: Disc,     label: "Front Disc Brake",  desc: "Reliable front disc braking"   },
...
const galleryImages = [
  { src: "/vehicles/megna-plus/megna-plus-ev-front.jpeg", label: "Front View"    },
  { src: "/vehicles/megna-plus/MegnaPlus-Rear.webp",      label: "Rear View"     },
  { src: "/vehicles/megna-plus/MegnaPlus-Side.webp",      label: "Side View"     },
  { src: "/vehicles/megna-plus/megna-plus-ev-colors.webp",label: "Color Options" },
];

export default function MegnaPlusPage() {
      ...
      <section ref={heroRef} className={styles.hero}>
        <motion.div style={{ opacity: heroOpacity }} className={styles.heroLeft}>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.15, ease: [0.16, 1, 0.3, 1] }} className={styles.heroHeadBlock}>
            <h1 className={styles.heroTitle}>Megna Plus</h1>
            <p className={styles.heroTagline}>Choose comfort, choose more.</p>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className={styles.heroDesc}>
            Everything the Megna does, in a little more. A USB charging port, reverse gear and a front disc brake — small upgrades that make the daily commute genuinely smarter.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.55 }} className={styles.heroCtas}>
            <a href="https://wa.me/919876543210?text=Hi%2C+I%27m+interested+in+the+Megna+Plus+EV" target="_blank" rel="noopener noreferrer" className={styles.ctaWhatsapp}>
        ...
        <div className={styles.heroRight}>
          <div className={styles.tiltScene} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <motion.div ref={cardRef} style={{ scale: heroImgScale, rotateX, rotateY }} className={styles.heroImgInner}>
              <Image src="/vehicles/megna-plus/megnaplusev.webp" alt="Megna Plus EV" fill priority className={styles.heroImg} sizes="55vw" quality={90} />
      ...
      <section className={styles.specsSection}>
        <div className="container-xl">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.whoForCard}>
            ...
            <p className={styles.whoForText}>The budget-conscious buyer who wants the essentials plus a couple of genuinely useful comforts, without splashing up to the premium line.</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.galleryHeading}>
            <div>
              ...
              <h2 className={styles.galleryTitle}>See the <span className={styles.accent}>Megna Plus</span> up close.</h2>
            </div>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.ctaBox}>
            <div className={styles.ctaBoxLeft}>
              ...
              <h2 className={styles.ctaBoxTitle}>Book your <span className={styles.accent}>Megna Plus</span> today.</h2>
