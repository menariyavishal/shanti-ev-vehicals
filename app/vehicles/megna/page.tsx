...
import { ArrowLeft, Feather, Disc, Package, Shield, Bell, ChevronRight, MessageCircle, Camera } from "lucide-react";
import styles from "../supreme/supreme.module.css";

const keyFeatures = [
  { icon: Feather, label: "Light 60Kg Body",    desc: "Lightweight & easy to handle"   },
  { icon: Disc,    label: "Front Disc Brake",   desc: "Reliable front disc braking"    },
  { icon: Package, label: "Tubeless Tyres",     desc: "Puncture-resistant performance" },
...
const specGroups = [
  ...
  },
  {
    ...
    items: [
      { label: "Brake System", value: "F-Disc / R-Drum"       },
...
const galleryImages = [
  { src: "/vehicles/megna/Megna-Front.webp",   label: "Front View"   },
  { src: "/vehicles/megna/Megna-Back.webp",    label: "Rear View"    },
  { src: "/vehicles/megna/Megna-Side.jpeg",    label: "Side View"    },
  { src: "/vehicles/megna/Megna-Closeup.webp", label: "Close Up"     },
];

export default function MegnaPage() {
      ...
      <section ref={heroRef} className={styles.hero}>
        <motion.div style={{ opacity: heroOpacity }} className={styles.heroLeft}>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.15, ease: [0.16, 1, 0.3, 1] }} className={styles.heroHeadBlock}>
            <h1 className={styles.heroTitle}>Megna</h1>
            <p className={styles.heroTagline}>Comfort bhi, style bhi.</p>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.35 }} className={styles.priceRow}>
              <span className={styles.priceAmount}>&#8377;39,999</span>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className={styles.heroDesc}>
            The budget hero. 60&nbsp;kg body, Front disc brake, tubeless tyres — the easiest, most honest EV designed for first-time buyers and families that do short, dependable trips every day.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.55 }} className={styles.heroCtas}>
            <a href="https://wa.me/919876543210?text=Hi%2C+I%27m+interested+in+the+Megna+EV" target="_blank" rel="noopener noreferrer" className={styles.ctaWhatsapp}>
        ...
        <div className={styles.heroRight}>
          <div className={styles.tiltScene} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <motion.div ref={cardRef} style={{ scale: heroImgScale, rotateX, rotateY }} className={styles.heroImgInner}>
              <Image src="/vehicles/megna/Megnane.webp" alt="Megna EV" fill priority className={styles.heroImg} sizes="55vw" quality={90} />
      ...
      <section className={styles.specsSection}>
        <div className="container-xl">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.whoForCard}>
            ...
            <p className={styles.whoForText}>The first-time buyer — a student, a homemaker, a son — who wants a cheap, dependable ride without RTO drama or a big wallet.</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.galleryHeading}>
            <div>
              ...
              <h2 className={styles.galleryTitle}>See the <span className={styles.accent}>Megna</span> up close.</h2>
            </div>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.ctaBox}>
            <div className={styles.ctaBoxLeft}>
              ...
              <h2 className={styles.ctaBoxTitle}>Book your <span className={styles.accent}>Megna</span> today.</h2>
