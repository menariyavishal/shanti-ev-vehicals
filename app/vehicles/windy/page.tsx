...
import { ArrowLeft, Disc, Package, Shield, Bell, Headphones, ChevronRight, MessageCircle, Camera } from "lucide-react";
import styles from "../supreme/supreme.module.css";

const keyFeatures = [
  { icon: Disc,       label: "Disc / Disc Brakes",    desc: "Front & rear disc braking"       },
  { icon: Package,    label: "Tubeless 12″ Wheels",   desc: "Puncture-resistant tubeless tyres"},
  { icon: Shield,     label: "100Kg-Rated Frame",     desc: "Robust high-strength chassis"    },
  { icon: Bell,       label: "Anti-Theft Alarm",      desc: "Built-in alarm protection"       },
  { icon: Headphones, label: "Around Service",        desc: "Pan-India service network"       },
];
...
const galleryImages = [
  { src: "/vehicles/windy/windy-ev-front.webp",  label: "Front View"    },
  { src: "/vehicles/windy/Windy-Rear.webp",      label: "Rear View"     },
  { src: "/vehicles/windy/Windy-Side.webp",      label: "Side View"     },
  { src: "/vehicles/windy/windy-ev-colors.jpeg", label: "Color Options" },
];
      ...
      <section ref={heroRef} className={styles.hero}>
        <motion.div style={{ opacity: heroOpacity }} className={styles.heroLeft}>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.15, ease: [0.16, 1, 0.3, 1] }} className={styles.heroHeadBlock}>
            <h1 className={styles.heroTitle}>Windy</h1>
            <p className={styles.heroTagline}>Ride the freedom.</p>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className={styles.heroDesc}>
            A balanced ride with a powerful BLDC motor, Disc brakes front and rear, a 100&nbsp;kg-rated frame and showroom colours — designed for those who demand the best for their price.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.55 }} className={styles.heroCtas}>
            <a href="https://wa.me/919876543210?text=Hi%2C+I%27m+interested+in+the+Windy+EV" target="_blank" rel="noopener noreferrer" className={styles.ctaWhatsapp}>
        ...
        <div className={styles.heroRight}>
          <div className={styles.tiltScene} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <motion.div ref={cardRef} style={{ scale: heroImgScale, rotateX, rotateY }} className={styles.heroImgInner}>
              <Image src="/vehicles/windy/windy-ev.webp" alt="Windy EV" fill priority className={styles.heroImg} sizes="55vw" quality={90} />
      ...
      <section className={styles.specsSection}>
        <div className="container-xl">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.whoForCard}>
            ...
            <p className={styles.whoForText}>The daily rider who wants premium-grade braking and a solid frame at the best-value price point.</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.galleryHeading}>
            <div>
              ...
              <h2 className={styles.galleryTitle}>See the <span className={styles.accent}>Windy</span> up close.</h2>
            </div>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.ctaBox}>
            <div className={styles.ctaBoxLeft}>
              ...
              <h2 className={styles.ctaBoxTitle}>Book your <span className={styles.accent}>Windy</span> today.</h2>
