...
import {
  ArrowLeft, Zap, Shield, RotateCcw, Usb, Droplets,
  ChevronRight, MessageCircle, Camera,
} from "lucide-react";
import styles from "../supreme/supreme.module.css";

const keyFeatures = [
  { icon: Droplets,  label: "Hydraulic Shockers", desc: "Smooth ride absorption"   },
  { icon: Shield,    label: "Anti-Theft Alarm",   desc: "Built-in alarm system"    },
  { icon: RotateCcw, label: "Disc / Disc Brakes", desc: "Front & rear disc brakes" },
  { icon: Zap,       label: "LED Headlamp",       desc: "Bright LED lighting"      },
...
const galleryImages = [
  { src: "/vehicles/tejas/tejas-ev-front.webp", label: "Front View"    },
  { src: "/vehicles/tejas/Tejas-Rear.webp",     label: "Rear View"     },
  { src: "/vehicles/tejas/Tejas-Side.jpeg",     label: "Side View"     },
  { src: "/vehicles/tejas/tejasev-color.webp",  label: "Color Options" },
];
      ...
      <section ref={heroRef} className={styles.hero}>
        <motion.div style={{ opacity: heroOpacity }} className={styles.heroLeft}>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.15, ease: [0.16, 1, 0.3, 1] }} className={styles.heroHeadBlock}>
            <h1 className={styles.heroTitle}>Tejas</h1>
            <p className={styles.heroTagline}>Feel the power.</p>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className={styles.heroDesc}>
            Premium wheel rim, disc brakes with hydraulic suspension and LED headlamp — a confident, good-looking choice for the discerning daily commuter.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.55 }} className={styles.heroCtas}>
            <a href="https://wa.me/919876543210?text=Hi%2C+I%27m+interested+in+the+Tejas+EV" target="_blank" rel="noopener noreferrer" className={styles.ctaWhatsapp}>
        ...
        <div className={styles.heroRight}>
          <div className={styles.tiltScene} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <motion.div ref={cardRef} style={{ scale: heroImgScale, rotateX, rotateY }} className={styles.heroImgInner}>
              <Image src="/vehicles/tejas/tejasev.webp" alt="Tejas EV" fill priority className={styles.heroImg} sizes="55vw" quality={90} />
      ...
      <section className={styles.specsSection}>
        <div className="container-xl">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.whoForCard}>
            ...
            <p className={styles.whoForText}>The Tier-2 working professional, 25–50, who commutes daily and wants a scooter that looks the part — without RTO paperwork or petrol bills.</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.galleryHeading}>
            <div>
              ...
              <h2 className={styles.galleryTitle}>See the <span className={styles.accent}>Tejas</span> up close.</h2>
            </div>
        ...
        <div className="container-xl">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className={styles.ctaBox}>
            <div className={styles.ctaBoxLeft}>
              ...
              <h2 className={styles.ctaBoxTitle}>Book your <span className={styles.accent}>Tejas</span> today.</h2>
